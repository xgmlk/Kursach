...
//call-back функция - ее можно вызвать с задержкой
//таким образом даже если функция 1 и 2 стоят друг за другом
//то если сделать из функции 1 call-back функцию то
//она вызовется после второй

function learnJS(lang,callback ) {
    console.log('учу ' + lang);
    callback();
}
...
function done() {
    console.log('я прошел 3й урок1');
}

learnJS1 ('JS', done);

//получилось тоже самое
// learnJS1 ('JS', done); сюда написал не вызов функции,
//done(){} , а саму функцию done чтобы она выполнилась

//callback нужен для того чтобы быть уверенным что код не 
//начнет свое исполнение пока другой код не завершится

//объекты

// let obj = new Object();
//это старый метод им никто не пользуется


//правильный вариант
//любой объект содержит пару ключ значений 
//напривер: яблоко - это фрукт. лук это овощь, а вода жидкая
//записывая это в объект , мы создаем свойства объекта
let options = {
    width: 1024,
    height: 1024,
    name: 'sanya'
...
//так же можно задавать свойтва объекту 
options.bool = false;
//тоже самое что и просто пописать это свойство вверху в объект
...
// bool удалится уже после того как его выведет консоль по этому 
//его будет видно в консоли и если снова вызвать 
console.log(options.bool);
// получим undefined ведь такого свойства уже нет

//цикл for in
for (let key322 in options) {
    console.log('Свойство ' + key322 + ' имеет значение ' + options[key322]);
}

//переменная key322(название) - обозначает каждое свойство или метод 
//который есть в объекте (ширина, высота итд)
//дальше параметр in(в) дальше тот объект который хотим перебрать options 
//как и в любом цикле фигурные скобки {}
//дальше пишем то что мы хотим сделать
//так как мы проходим по каждому свойству и методу объекта 
//вместо key322 будет попадать каждое свойство из объекта
//дальше (имеет значение + название объекта[переменная key322])
//через квадратные скобки тоже самое что и через точку получать 
//отдельное свойство(   console.log(options.name)   )

console.log(Object.keys(options).length);

//object - говорит что мы будет работать с объектом
//keys - значит что будем работать с его ключами (свойствами, методами...)
//lenght - узнаю количество ключей keys в объекте 


//массивы

let arr1 = [1,2,3,4,5];
//удаление последнего элемента массива т.е (5)
arr1.pop();
//добавление элемента в конец массива
//добавили 5 только уже в виде строки
arr1.push("5");
//shift и unshift
...
//каждый раз когда цикл for вызывается - у него запускается команда 
//console.log и перебирает каждый элемент массива по порядку
for (let i = 0; i < arr1.length; i++) {
    console.log(arr1[i]);
}


//перебор массиво второй способ forEach
arr1.forEach(function(item, i, mass) {
    console.log(i + ': ' + item) + ' (массив: ' + mass + ')';
});
//item это 1 2 3 итд в нашем массиве каждый элемент в массиве
//i - может быть любое название это номер нашего элемента
//mass - название массива 

//for off - не работает в объектах
...
//получили 01234 так как fot in перебирает пару ключ значение 
// у еденици ключ 0, а у тройки 1 итд
//let key in можно заменить на ley key of
// и мы получим именно те значения которые записанны в массиве 
//тоесть 13467
...
//новые элементы которые были записанны через запятую
//в сплит записывается разделитель тоесть пробел


//этот метод берет каждый элемент массива и скленивает все в одну строку
// в join так же записывается разделитель тоесть запятая и пробел
let rar = ['zaa','bvb', 'aasad']
let bo = rar.join(',', ' ');

//если записываем массив rar то его в консоли и получаем
//а если запишем bo тогда получится строка
//zaa, bvb, aasad 
    // console.log(rar);
    console.log(bo);

//если нужно отсортировать массив по альфавиту используется команда sort

let sos = ['zaa','bvb', 'aasad']
let c = sos.sort();

    console.log(sos);

//метод sort сравниевает по первым значениям
//т.е если есть 1 15 4 после sort они останутся на своих местах
//т.к в числе 15 первая цифра 1

//чтобы все сортировалось правильно нужно
